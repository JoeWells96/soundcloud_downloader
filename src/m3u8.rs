...
fn download_segments_to_file(playlist: MediaPlaylist, output_file: &File) {
    playlist.segments.iter().for_each(|seg| write_seg_to_file(download_seg(seg.uri.to_string()), output_file))
}
...
fn write_seg_to_file(bytes: Bytes, mut output_file: &File) {
    output_file.write_all(&bytes[..]).expect("Failed to write segment bytes to file")
}
